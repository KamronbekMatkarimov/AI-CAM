...
Connects to camera, captures a frame, releases the stream.
Includes basic protection from corrupted/black frames.
"""
...
def capture_frame(
    ...
    for attempt in range(1, retry_attempts + 1):
        ...
        try:
            ...
            if float(frame.mean()) < float(MIN_FRAME_MEAN):
                logger.warning(
                    "Bad frame (mean=%.1f < %d) - black/corrupted, skipping",
                    float(frame.mean()),
                    MIN_FRAME_MEAN,
                )
                continue

            logger.info("Frame captured successfully (mean=%.1f)", float(frame.mean()))
            return frame
